...
void HistoryManager::saveEntry(const TranslationEntry& entry) {
    ...
    if (file.open(QIODevice::WriteOnly)) {
        ...
        file.close();
        // Update cache
        m_markdownCache[entry.id] = HistoryManager::normalizeMarkdown(entry.translatedMarkdown);
    }
...
TranslationEntry HistoryManager::getEntryById(const QString& id) {
    QList<TranslationEntry> entries = loadEntries();
...
bool HistoryManager::addTagsToEntries(const QStringList& ids, const QStringList& tags) {
    ...
    if (ids.isEmpty() || tags.isEmpty()) return true;
    m_ignoreNextChange = true;
    QFile file(getJsonPath());
    if (!file.exists() || !file.open(QIODevice::ReadOnly)) {
        qDebug() << "Failed to open history.json for reading";
...
QStringList HistoryManager::getAllTags() {
    ...
    if (!doc.isArray()) return QStringList();
...
void HistoryManager::onFileChanged(const QString& path) {
    if (m_ignoreNextChange) return;
    
    // Reload entries and diff with cache
    QFile file(path);
    ...
    for (const auto& val : array) {
        ...
        QString rawMd = obj["markdown"].toString();
        QString normMd = HistoryManager::normalizeMarkdown(rawMd);
        
        if (m_markdownCache.contains(id)) {
            if (m_markdownCache[id] != normMd) {
                m_markdownCache[id] = normMd;
                emit entryMarkdownChanged(id, normMd);
            }
        } else {
            // New entry added externally
            m_markdownCache[id] = normMd;
