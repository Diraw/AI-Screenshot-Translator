...
ResultWindow::ResultWindow(QWidget *parent) : QWidget(parent) {
    ...
    m_webView->bind("cmd_updateStatus", [this](std::string seq, std::string req, void* arg) {
        ...
        QMetaObject::invokeMethod(this, [this, labelText](){ if (m_statusLabel) m_statusLabel->setText(labelText); }, Qt::QueuedConnection);
    });
...
void ResultWindow::setContent(const QString &markdown, const QString &originalBase64, const QString &prompt, const QString &entryId) {
    ...
    if (m_history.isEmpty() && !entryId.isEmpty()) {
        TranslationEntry e; e.id = entryId; e.translatedMarkdown = markdown; e.originalBase64 = originalBase64; e.prompt = prompt;
        m_history.append(e); m_currentIndex = 0;
        updateNavigation();
    }
...
window.toggleSource = function() {
  ...
  else { r.style.display='none'; c.style.display='block'; if(window.cmd_updateStatus) window.cmd_updateStatus('view'); }
};
window.toggleEdit = function() {
  var c = document.getElementById('content'), e = document.getElementById('edit_view'), r = document.getElementById('raw_view');
  if (!EDIT) { EDIT=true; e.value=CUR_MD; e.style.display='block'; c.style.display='none'; r.style.display='none'; e.focus(); if(window.cmd_updateStatus) window.cmd_updateStatus('edit'); }
  else { EDIT=false; CUR_MD=e.value; if(window.cmd_updateContent) window.cmd_updateContent(CUR_MD); render(CUR_MD); e.style.display='none'; c.style.display='block'; if(window.cmd_updateStatus) window.cmd_updateStatus('view'); document.getElementById('sink').focus(); }
};
...
document.addEventListener('DOMContentLoaded', () => { render(CUR_MD); document.getElementById('sink').focus(); });
</script></body></html>
)RAW_HTML";
    ...
    QString out = html
        ...
        .arg(toUri(loadAsset("katex.min.css"), "text/css"))
...
void ResultWindow::configureHotkeys(const QString &v, const QString &e, const QString &s, const QString &b, const QString &u, const QString &h, const QString &p, const QString &n, const QString &t) {
    m_viewToggleKey = v; m_editToggleKey = e; m_screenshotToggleKey = s; 
    m_boldKey = b; m_underlineKey = u; m_highlightKey = h;
    m_prevKey = p; m_nextKey = n; m_tagKey = t;
    qDeleteAll(m_navShortcuts); m_navShortcuts.clear();
    auto add = [this](const QString& k, std::function<void()> f, Qt::ShortcutContext c = Qt::WindowShortcut) {
        if (k.isEmpty()) return;
        QShortcut *sc = new QShortcut(QKeySequence(k), this); sc->setContext(c);
        connect(sc, &QShortcut::activated, this, f); m_navShortcuts.append(sc);
    };
    add(m_viewToggleKey, [this](){ if(m_webView) m_webView->eval("toggleSource();"); }, Qt::ApplicationShortcut);
    add(m_editToggleKey, [this](){ if(m_webView) m_webView->eval("toggleEdit();"); }, Qt::ApplicationShortcut);
    add(m_screenshotToggleKey, [this](){ if(m_webView) m_webView->eval("toggleScreenshot();"); }, Qt::ApplicationShortcut);
    add(m_boldKey, [this](){ if(m_webView) m_webView->eval("applyFormat('bold');"); });
    add(m_underlineKey, [this](){ if(m_webView) m_webView->eval("applyFormat('underline');"); });
    add(m_highlightKey, [this](){ if(m_webView) m_webView->eval("applyFormat('highlight');"); });
    add(m_prevKey, [this](){ showPrevious(); });
    add(m_nextKey, [this](){ showNext(); });
    add(m_tagKey, [this](){ openTagDialog(); });
}

void ResultWindow::addEntry(const TranslationEntry& entry) {
    m_history.append(entry); m_currentIndex = m_history.size() - 1;
    updateNavigation(); setContent(entry.translatedMarkdown, entry.originalBase64, entry.prompt, entry.id);
}
void ResultWindow::showPrevious() { if (m_currentIndex > 0) { m_currentIndex--; const auto& e = m_history[m_currentIndex]; setContent(e.translatedMarkdown, e.originalBase64, e.prompt, e.id); updateNavigation(); } }
void ResultWindow::showNext() { if (m_currentIndex < m_history.size() - 1) { m_currentIndex++; const auto& e = m_history[m_currentIndex]; setContent(e.translatedMarkdown, e.originalBase64, e.prompt, e.id); updateNavigation(); } }
void ResultWindow::updateNavigation() {
    m_prevAction->setEnabled(m_currentIndex > 0); m_nextAction->setEnabled(m_currentIndex < m_history.size() - 1);
...
void ResultWindow::resizeEvent(QResizeEvent *event) {
    if (m_webContainer) m_webContainer->setGeometry(0, 0, width(), height());
    if (m_webView) m_webView->setSize(width(), height());
    if (m_toolBar) { m_toolBar->setGeometry(0, 0, width(), 46); m_toolBar->raise(); }
    QSizeGrip *grip = findChild<QSizeGrip*>();
    if (grip) { grip->move(width() - grip->width(), height() - grip->height()); grip->raise(); }
    QWidget::resizeEvent(event);
}
void ResultWindow::showEvent(QShowEvent *event) {
    setUpdatesEnabled(false);
    QTimer::singleShot(2000, this, [this]() { if (!updatesEnabled()) setUpdatesEnabled(true); }); 
    QWidget::showEvent(event);
    QTimer::singleShot(0, this, [this]() { this->raise(); this->activateWindow(); this->setFocus(); focusEditor(); if (m_webView) m_webView->eval("document.body.focus();"); });
}
void ResultWindow::closeEvent(QCloseEvent *event) { emit closed(); event->accept(); }
bool ResultWindow::event(QEvent *event) { return QWidget::event(event); }
void ResultWindow::setHistoryManager(HistoryManager* h) { m_historyManager = h; }
void ResultWindow::focusEditor() { if (m_webView) m_webView->eval("document.getElementById('sink').focus();"); }
void ResultWindow::setConfig(const AppConfig &config) { m_config = config; }
ResultWindow::ProtectedContent ResultWindow::protectMath(const QString &m) { ProtectedContent p; p.text = m; return p; }
...
void ResultWindow::externalContentUpdate(const QString &m) {
    m_currentMarkdown = m;
    if (m_webView) {
        QString js = m; js.replace("\\", "\\\\").replace("\"", "\\\"").replace("\n", "\\n").replace("\r", "");
...
void ResultWindow::openTagDialog() {
    if (!m_historyManager || m_entryId.isEmpty()) return;
