...
class ConfigApp(QWidget):
    ...
    def load_config_to_ui(self):
        ...
        if not config_data:
            ...
            QMessageBox.information(self, "提示", "未检测到 config.yaml 文件，已加载默认配置。请填写后保存。")
    ...
    def save_config_from_ui(self):
        ordered_config_to_save = copy.deepcopy(self.config_manager.get_default_config())
        self._update_config_from_gui_widgets(ordered_config_to_save)

        try:
            if self.config_manager.save_config(ordered_config_to_save):
                ...
                if full_key_path in self.config_widgets:
                    ...
                    try:
                        if isinstance(widget, QLineEdit):
                            ...
                            config_dict[key] = str(widget.text())
                    except ValueError:
    ...
    def load_hint_content(self):
        # 将文件扩展名从 .html 改为 .md
        hint_file_path = os.path.join(os.path.dirname(__file__), "hint.md")
        if os.path.exists(hint_file_path):
            try:
                ...
                self.hint_text_edit.setPlainText(f"无法加载提示文件 'hint.md'：\n{e}")
                QMessageBox.warning(self, "错误", f"无法加载提示文件 'hint.md'：\n{e}")
        else:
            self.hint_text_edit.setPlainText("未找到提示文件 'hint.md'。请确保它与应用程序在同一目录下。")
            QMessageBox.information(self, "提示", "未找到提示文件 'hint.md'。")
